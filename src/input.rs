...
impl Input{
    pub fn new() -> Self{ Self{ target_value: 0.0, acceptable_time: 0.0} }
...
impl ConsoleInput for Input{
    fn type_in() -> Input{
        ...
        let target_value = value.trim().parse()
        ...
        let acceptable_time = time.trim().parse()
            .expect("Please enter a valid number of seconds.");

        Self{ target_value, acceptable_time }
    }
}